    ...
    <div className={`bg-zinc-900 border border-zinc-800 rounded-xl p-5 flex flex-col gap-3 group ${theme.active ? '' : 'opacity-60'}`}>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button size="sm" variant="ghost" onClick={onToggleActive}
                className={theme.active ? 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20' : 'bg-zinc-800 text-zinc-500 hover:bg-zinc-700'}>
                {theme.active ? 'ON' : 'OFF'}
              </Button>
            </TooltipTrigger>
            <TooltipContent>{theme.active ? 'Disable theme' : 'Enable theme'}</TooltipContent>
          </Tooltip>
          <Button size="icon-sm" variant="ghost" onClick={onEdit} className="text-zinc-600 hover:text-zinc-300 opacity-0 group-hover:opacity-100">✏️</Button>
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {tags.map(t => (
            <span key={t} className="text-[10px] px-1.5 py-0.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-full">
              {t}
            </span>
          ))}
        </div>
      ...
      {theme.validationCriteria && (
        <p className="text-[11px] text-zinc-600 italic border-t border-zinc-800 pt-2">{theme.validationCriteria}</p>
      )}
    </div>
  )
}