...
import { renderNote } from './actions'
...
export default function NotesClient({ notes }: { notes: Note[] }) {
  ...
  async function handleSelect(note: Note) {
    setSelected(note)
    setRendering(true)
    const rendered = await renderNote(note.content)
    setHtml(rendered)
    setRendering(false)
  }
  ...
  if (selected) return (
    <div>
      <button onClick={() => { setSelected(null); setHtml(null) }}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
        ...
        {rendering
          ? <div className="text-zinc-600 text-sm animate-pulse">Rendu...</div>
