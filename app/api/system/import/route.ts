...
export async function POST(req: NextRequest) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // Prompts
    const promptStats: EntityStats = { created: 0, updated: 0, skipped: 0 }
        ...
        await tx.prompt.create({
          data: {
            ...
            messages: messages?.length
              ? { create: (messages as any[]).map((m: any, i: number) => ({ role: m.role, content: m.content, order: i })) }
              : undefined,
          },
        })
        promptStats.created++
      } else {
        const r = await tx.prompt.updateMany({
          where: { id: p.id, ...needsUpdate(existing, data) ? {} : { id: '__never__' } },
          data,
        })
        // Always sync messages on update
        if (messages !== undefined) {
          await tx.promptMessage.deleteMany({ where: { promptId: p.id } })
          if (messages.length) {
            await tx.promptMessage.createMany({
              data: (messages as any[]).map((m: any, i: number) => ({ promptId: p.id, role: m.role, content: m.content, order: i })),
            })
          }
        }
        if (r.count > 0) promptStats.updated++; else promptStats.skipped++
      }
    }
